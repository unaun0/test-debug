//
//  AttendanceRepositoryFindITCase.swift
//  Backend
//
...
final class AttendanceRepositoryFindITCase: XCTestCase {
    ...
    func testFindAttendanceById_Success() async throws {
        ...
        let foundAttendance = try await sut.find(id: attendance.id)
        
        XCTAssertNotNil(foundAttendance)
        XCTAssertEqual(foundAttendance?.status, attendance.status)
        XCTAssertEqual(foundAttendance?.membershipId, attendance.membershipId)
        XCTAssertEqual(foundAttendance?.trainingId, attendance.trainingId)
    }
    
    func testFindAttendanceById_NotFound() async throws {
        let foundAttendance = try await sut.find(id: UUID())
            
        XCTAssertNil(foundAttendance)
    }
}
