...
final class AttendanceRepositoryDeleteITCase: XCTestCase {
    ...
    func testDeleteAttendance_NotFound() async throws {
        do {
            ...
            XCTFail("Expected AttendanceRepositoryError but none was thrown")
        } catch let error as AttendanceRepositoryError {
            XCTAssertEqual(AttendanceRepositoryError.attendanceNotFound, error)
        } catch {
