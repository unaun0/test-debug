...
public final class IMembershipTypeRepositoryMock: IMembershipTypeRepository, @unchecked Sendable {
    public init() { }


    private let createState = MockoloMutex(MockoloHandlerState<Never, @Sendable (MembershipType) async throws -> ()>())
    public var createCallCount: Int {
        return createState.withLock(\.callCount)
    }
    public var createHandler: (@Sendable (MembershipType) async throws -> ())? {
    ...
    public func create(_ membershipType: MembershipType) async throws {
        ...
        if let createHandler = createHandler {
            try await createHandler(membershipType)
        }
        
    }

    private let updateState = MockoloMutex(MockoloHandlerState<Never, @Sendable (MembershipType) async throws -> ()>())
    public var updateCallCount: Int {
        return updateState.withLock(\.callCount)
    }
    public var updateHandler: (@Sendable (MembershipType) async throws -> ())? {
    ...
    public func update(_ membershipType: MembershipType) async throws {
        ...
        if let updateHandler = updateHandler {
            try await updateHandler(membershipType)
        }
